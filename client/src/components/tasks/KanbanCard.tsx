...
const PRIORITY_CONFIG: Record<number, any> = {
...
function formatDate(dateStr: string | null) {
  ...
  if (diff < 0) return { text: 'Atrasada', className: 'text-red-400 bg-red-500/[0.08] border border-red-500/10' }
  if (diff === 0) return { text: 'Hoje', className: 'text-emerald-400 bg-emerald-500/[0.08] border border-emerald-500/10' }
  if (diff === 1) return { text: 'Amanhã', className: 'text-orange-400 bg-orange-500/[0.08] border border-orange-500/10' }
  return {
...
export default function KanbanCard({ task, dragListeners, isDraggingOverlay, isPlaceholder }: KanbanCardProps) {
  ...
  const cardColor = (task as any)?.creator?.theme_color || '#7c3aed'
          ...
          <div className="flex items-start gap-3">
            ...
            >
              {isCompleted ? (
                ...
                <Circle size={18} className="text-slate-300 hover:text-purple-400 transition-colors" />
              )}
            </button>
            <p
          ...
          {(task.task_labels || []).length > 0 && (
            <div className="mt-2.5 ml-[30px] flex flex-wrap gap-1.5">
              {task.task_labels!.map((tl: any) => (
                <span
                  key={tl.id || (typeof tl === 'string' ? tl : JSON.stringify(tl))}
              ...
              {!isCompleted && subtasks.map((sub) => {
                ...
                return (
                  <div
                    ...
                    className="flex cursor-pointer items-center gap-2 transition-all hover:bg-white/5 py-0.5"
                  >
                    {done ? (
