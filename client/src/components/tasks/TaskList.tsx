...
import { useTasks, useGlobalKPIs, Task } from '../../hooks/useTasks'
...
export default function TaskList({ projectId, title, filterToday }: TaskListProps) {
  const { user } = useAuth()
    ...
    <div className="space-y-12 animate-in fade-in duration-700">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 p-1 rounded-[22px] border border-white/[0.03] bg-black/10">
        <div className="dark-neo-recessed p-8 flex flex-col gap-6 relative overflow-hidden group hover:border-white/10 transition-all duration-500">
          ...
          <p className="text-4xl font-black text-white tracking-tighter relative z-10" style={{ textShadow: '0 0 20px rgba(255,255,255,0.1)' }}>{kpis.velocidade_media}</p>
        </div>
        ...
        <div className="dark-neo-recessed p-8 flex flex-col gap-6 relative overflow-hidden group hover:border-white/10 transition-all duration-500">
          ...
          <div className="flex items-center gap-4 relative z-10">
            <div className={`rounded-2xl p-3.5 shadow-lg ${kpis.atencao_critica > 0 ? 'bg-orange-600 text-white shadow-orange-600/20' : 'bg-[#151515] text-slate-600'}`}>
          ...
          <div className="flex items-baseline gap-3 relative z-10">
            <p className="text-4xl font-black text-white tracking-tighter">{kpis.atencao_critica}</p>
        ...
        <div className="dark-neo-recessed p-8 flex flex-col gap-6 relative overflow-hidden group hover:border-white/10 transition-all duration-500">
          ...
          <div className="flex items-baseline gap-3 relative z-10">
            <p className="text-4xl font-black text-white tracking-tighter">{kpis.volume_atribuido}</p>
