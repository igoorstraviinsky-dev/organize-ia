...
import { Plus, CalendarDays, Flag, Clock, Tag } from 'lucide-react'
...
export default function TaskForm({ projectId, sectionId, parentId, onClose }: TaskFormProps) {
  ...
  const { data: sections = [] } = useSections(activeProjectId || '')
        ...
        <div className="relative">
          ...
          {showLabels && labels.length > 0 && (
            <>
              ...
              <div className="absolute left-0 z-20 mt-2 w-56 rounded-xl border border-slate-700 bg-slate-900 p-2 shadow-xl">
                <div className="max-h-48 overflow-y-auto">
                  {labels.map((label: any) => (
                    <button
                      ...
                      className="flex w-full items-center gap-3 rounded-xl px-3 py-2 text-[11px] font-bold text-slate-300 transition-all hover:bg-white/5"
                    >
                      <span
                        className={`h-3 w-3 rounded-full border-2 transition-transform ${selectedLabelIds.includes(label.id) ? 'scale-110 border-transparent' : 'border-slate-700'}`}
