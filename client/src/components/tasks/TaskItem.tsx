...
import { CalendarDays, Flag, Trash2, Hash, Clock, CheckCircle2, MessageSquare } from 'lucide-react'
...
const PRIORITY_LABELS: Record<number, string> = { 1: 'Urgente', 2: 'Alta', 3: 'Média', 4: 'Normal' }
...
export default function TaskItem({ task }: TaskItemProps) {
  ...
  const isCold = !isCompleted && ((Date.now() - new Date(task.updated_at || task.created_at).getTime()) / (1000 * 60 * 60) > 48)
  const cardColor = (task as any)?.creator?.theme_color || '#7c3aed'
  const commentsCount = (task as any).comments?.length || 0
  
  let originBadge: string | null = null;
  if (user?.id) {
