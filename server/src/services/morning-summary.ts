...
interface UserSummarySettings {
  user_id: string;
  morning_summary_times: string[] | null;
  profiles: {
    full_name: string;
    phone: string;
  } | null;
...
async function checkAndSendSummaries() {
  ...
  const typedSettings = usersSettings as unknown as UserSummarySettings[];

  // 2. Filtra quem agendou para o horário de Brasília atual
  const usersToNotif = typedSettings.filter(settings => {
...
export async function processUserSummary(settings: UserSummarySettings) {
  ...
  // Nota: 'completed_at::date' é sintaxe de casting do PostgREST/Supabase em alguns contextos,
  // mas aqui simplificamos a lógica para carregar e filtrar.
  const [
    { data: tasksToday },
    { data: tasksOverdue },
    ...
    supabase.from('tasks').select('title').eq('creator_id', userId).gte('completed_at', yesterday).eq('status', 'completed')
  ]);
