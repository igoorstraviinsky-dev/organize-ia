...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (message.type === 'text') {
      ...
      const { downloadMediaAsBase64 } = await import('../lib/whatsapp.js');
      const base64Image = await downloadMediaAsBase64(mediaId);
    ...
    const response = await processMessage(userText as string, from);
    await sendWhatsAppMessage(from, response);
